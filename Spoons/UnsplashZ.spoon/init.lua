...
local function curl_callback(exitCode, stdOut, stdErr)
	if exitCode == 0 then
		...
		local localpath = os.getenv("TMPDIR") .. hs.http.urlParts(obj.pic_url).lastPathComponent
...
local function unsplashRequest()
	...
	obj.pic_url = hs.execute(
		[[ /usr/bin/curl 'https://source.unsplash.com/3840x2160/?nature' |  perl -ne ' print "$1" if /href="([^"]+)"/ ' ]]
	)
